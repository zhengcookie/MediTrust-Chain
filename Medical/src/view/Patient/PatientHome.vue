  ...
  <div id="base">
    ...
    <div class="sidebar">
      ...
      <div class="menu-item" @click="router.push('/patient/reserveinfo')">
        ...
        <router-link to="/patient/reserveinfo">预约管理</router-link>
      </div>
<!-- 
...
const username = JSON.parse(localStorage.getItem("patientinfo") || '{"username":"未登录"}').username;

// 在组件挂载时检查用户是否登录
onMounted(() => {
  const patientInfo = localStorage.getItem("patientinfo");
  if (!patientInfo || JSON.parse(patientInfo).username === "未登录") {
    router.push("/login");
  }
});
